...
.DESCRIPTION
    ...
    Input is alert data with information needed to identify which VM to delete.
...
{ 
    ...
    if (($status -eq "Activated") -or ($status -eq "Fired"))
    {
        ...
        if ($ResourceType -eq "Microsoft.Compute/virtualMachines")
        {
            ...
            # Delete the ARM VM  
            Write-Verbose "Deleting the VM - $ResourceName - in resource group - $ResourceGroupName -" -Verbose
            Remove-AzVM -Name $ResourceName -ResourceGroupName $ResourceGroupName -DefaultProfile $AzureContext -Force
            # [OutputType("PSAzureOperationResponse")]
        }
        else {
